...
double goals::timeToReach2D_Ex(const Vec3d& from, const Vec3d& target, const Vec3d& v0, double acceleration, double maxSpeed)
{
    Vec2d fromXZ         = { from.x, from.z };
    Vec2d targetXZ       = { target.x, target.z };
    Vec2d displacementXZ = targetXZ - fromXZ;
    Vec2d directionXZ    = linalg::normalize(displacementXZ);

    Vec2d v0_XZ          = Vec2d{ v0.x, v0.z };
    double approachSpeed = linalg::dot(v0_XZ, directionXZ);
    double distanceXZ    = linalg::length(displacementXZ);

    return uniform_accel::distanceToTime(distanceXZ, acceleration, approachSpeed, maxSpeed);
}

double goals::timeToReach3D_Ex(const Vec3d& from, const Vec3d& target, const Vec3d& v0, const model::Rules& rules)
{
    double time2D = timeToReach2D_Ex(from, target, v0, rules.ROBOT_ACCELERATION, rules.ROBOT_MAX_GROUND_SPEED);

    double liftAmount = target.y - from.y;
    if(liftAmount <= 0)
        return time2D;

    Vec2d displacementXZ = Vec2d(target.x, target.z) - Vec2d(from.x, from.z);
    Vec2d directionXZ    = linalg::normalize(displacementXZ);

    double v0_2D          = linalg::dot(Vec2d{ v0.x, v0.z }, directionXZ);
    ...
    double takeoffTick   = std::max(0.0, (time2D - secondsToLift) * rules.TICKS_PER_SECOND);
    double takeoffSecond = takeoffTick * tickTime;

    double takeoffSpeed = uniform_accel::timeToSpeed(v0_2D, acceleration2D, takeoffSecond, rules.ROBOT_MAX_GROUND_SPEED);
    double runawayLength = uniform_accel::timeToDistance(v0_2D, acceleration2D, takeoffSecond, rules.ROBOT_MAX_GROUND_SPEED);
    double runawaySeconds = uniform_accel::distanceToTime(runawayLength, acceleration2D, v0_2D, rules.ROBOT_MAX_GROUND_SPEED);
    double dbg_check = (int)std::abs(runawaySeconds - takeoffSecond);
    ...
    while(flyingSeconds > secondsToLift && (rules.ROBOT_MAX_GROUND_SPEED - std::abs(takeoffSpeed)) > k_Epsilon)
    {
        ...
        takeoffSpeed  = uniform_accel::timeToSpeed(v0_2D, acceleration2D, takeoffSecond, rules.ROBOT_MAX_GROUND_SPEED);
        runawayLength = uniform_accel::timeToDistance(v0_2D, acceleration2D, takeoffSecond, rules.ROBOT_MAX_GROUND_SPEED);

        flyingDistance = distance2D - runawayLength;
        flyingSeconds  = flyingDistance / takeoffSpeed;
    }

    return takeoffSecond + flyingSeconds;
}
